...
contract PandaToken is ERC20, Ownable {
    ...
    constructor(
        ...
        // console.log("\nConstructor set balanceOf ZeroAddress: ",balanceOf(address(0)));
    }
    ...
    function getTokens(uint amount, bytes memory signature) external {
        ...
        // console.log("\ngiftFrom ( %s ) ",giftFrom);
        // console.log("\t balanceOf : ",balanceOf(giftFrom));
        // console.log("\t burnPending init: ",burnPending[giftFrom]);

        burnPending[giftFrom] += amount;
        // console.log("\t burnPending after: ",burnPending[giftFrom]);
        require(amount == 1 ether, "amount error");
        // console.log("\t {%s} - {%s} = {%s}: ",balanceOf(giftFrom) , burnPending[giftFrom], balanceOf(giftFrom) - burnPending[giftFrom]);
        require(
        ...
        console.log("Minting {%s} Tokens , To : {%s}",amount,msg.sender);
        _mint(msg.sender, amount);
    }
